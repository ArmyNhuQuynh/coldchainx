...
const ReimbursementPage = () => {
  ...
  const expenseIncidents = useMemo(() => {
    const keyword = search.trim().toLowerCase();
    return (incidentsQuery.data ?? []).filter((incident) => {
      const hasExpense =
        incident.driverPaidAmount > 0 ||
        incident.expenseStatus !== INCIDENT_EXPENSE_STATUS.NOT_REQUESTED;
      const matchesSearch =
        !keyword ||
        [incident.incidentId, incident.tripId, incident.reportedByUsername, incident.description]
          ...
          .toLowerCase()
          .includes(keyword);
      return hasExpense && matchesSearch;
    });
  }, [incidentsQuery.data, search]);

  const pending = expenseIncidents.filter((item) => item.expenseStatus === INCIDENT_EXPENSE_STATUS.PENDING_REVIEW);
  const approved = expenseIncidents.filter((item) => item.expenseStatus === INCIDENT_EXPENSE_STATUS.APPROVED);
  const history = expenseIncidents.filter(
    (item) =>
      item.expenseStatus === INCIDENT_EXPENSE_STATUS.REIMBURSED ||
      item.expenseStatus === INCIDENT_EXPENSE_STATUS.REJECTED
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid overflow-hidden rounded-lg border bg-background md:grid-cols-3">
        ...
        <div className="flex min-h-24 items-center justify-between border-t p-4 md:border-l md:border-t-0"><div><p className="text-sm text-muted-foreground">Đã xử lý</p><p className="mt-1 text-2xl font-semibold">{history.length}</p><p className="text-xs text-muted-foreground">Hoàn tiền hoặc từ chối</p></div><CheckCircle2 className="h-5 w-5 text-emerald-700" /></div>
      </div>
      ...
      {incidentsQuery.isError ? (
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList>
            ...
            <TabsTrigger value="history">Đã xử lý ({history.length})</TabsTrigger>
          </TabsList>
          <TabsContent value="pending"><ExpenseTable incidents={pending} isLoading={incidentsQuery.isLoading} onView={setDetailIncident} onReview={setReviewIncident} onReimburse={setReimburseIncident} /></TabsContent>
          <TabsContent value="approved"><ExpenseTable incidents={approved} isLoading={incidentsQuery.isLoading} onView={setDetailIncident} onReview={setReviewIncident} onReimburse={setReimburseIncident} /></TabsContent>
          <TabsContent value="history"><ExpenseTable incidents={history} isLoading={incidentsQuery.isLoading} onView={setDetailIncident} onReview={setReviewIncident} onReimburse={setReimburseIncident} /></TabsContent>
        </Tabs>
      )}
