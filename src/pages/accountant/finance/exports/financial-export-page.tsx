  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        ...
        <section className="flex flex-col rounded-lg border bg-card p-5"><h2 className="text-lg font-semibold">Bảng kê COD</h2><p className="mt-1 min-h-10 text-sm text-muted-foreground">Bảng kê chuyến và tài xế trong kỳ theo dữ liệu export hiện tại của BE.</p><div className="mt-5"><label className="mb-1.5 block text-sm font-medium">Driver ID (không bắt buộc)</label><Input value={driverId} onChange={(event) => setDriverId(event.target.value)} placeholder="UUID tài xế" />{!validDriverId && <p className="mt-1 text-xs text-rose-600">Driver ID không đúng định dạng UUID.</p>}</div><Button type="button" className="mt-5 gap-2" disabled={!validRange || !validDriverId || exportCodSettlement.isPending} onClick={() => runExport(() => exportCodSettlement.mutateAsync({ ...baseParams, driverId: driverId.trim() || undefined }), "Đã tải bảng kê COD.")}><Download className="h-4 w-4" />Xuất COD</Button></section>
